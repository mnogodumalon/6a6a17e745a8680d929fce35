...
export default function KurseWorkshopsPage() {
  ...
  const [records, setRecords] = useState<KurseWorkshops[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<KurseWorkshops | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<KurseWorkshops | null>(null);
  ...
  async function handleCreate(fields: KurseWorkshops['fields']) {
  ...
  async function handleUpdate(fields: KurseWorkshops['fields']) {
    ...
    await LivingAppsService.updateKurseWorkshop(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteKurseWorkshop(deleteTarget.record_id);
