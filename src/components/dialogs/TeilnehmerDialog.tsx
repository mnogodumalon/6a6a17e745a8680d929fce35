/**
 * TeilnehmerDialog — pre-generated create/edit dialog for Teilnehmer.
 *
 ...
 * TeilnehmerDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Teilnehmer['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<TeilnehmerDialogDefaults | undefined>(undefined)
 */
...
/** Widened prefill type for TeilnehmerDialog.defaultValues — see file header. */
export type TeilnehmerDialogDefaults = Teilnehmer['fields'];
...
interface TeilnehmerDialogProps {
  ...
  defaultValues?: TeilnehmerDialogDefaults;
...
export function TeilnehmerDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: TeilnehmerDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Teilnehmer['fields']>);
      setPreview(null);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('teilnehmer') })
    : t('new_entity', { entity: appLabel('teilnehmer') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        <Label htmlFor="vorname">{fieldLabel('teilnehmer', 'vorname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="vorname"
          placeholder=""
    ...
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        <Label htmlFor="nachname">{fieldLabel('teilnehmer', 'nachname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="nachname"
          placeholder=""
    ...
    'geburtsdatum': (
      <div key="geburtsdatum" className="space-y-1.5">
        <Label htmlFor="geburtsdatum">{fieldLabel('teilnehmer', 'geburtsdatum')}</Label>
        <DatePicker
          id="geburtsdatum"
          placeholder=""
          mode="date"
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('teilnehmer', 'email')}</Label>
        <Input
          id="email"
          type="email"
          placeholder=""
    ...
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('teilnehmer', 'telefon')}</Label>
        <Input
          id="telefon"
    ...
    'postleitzahl': (
      <div key="postleitzahl" className="space-y-1.5">
        <Label htmlFor="postleitzahl">{fieldLabel('teilnehmer', 'postleitzahl')}</Label>
        <Input
          id="postleitzahl"
          placeholder=""
    ...
    'notfall_name': (
      <div key="notfall_name" className="space-y-1.5">
        <Label htmlFor="notfall_name">{fieldLabel('teilnehmer', 'notfall_name')}</Label>
        <Input
          id="notfall_name"
          placeholder=""
          value={fields.notfall_name ?? ''}
          onChange={e => setFields(f => ({ ...f, notfall_name: e.target.value }))}
        />
      </div>
    ),
    'notfall_telefon': (
      <div key="notfall_telefon" className="space-y-1.5">
        <Label htmlFor="notfall_telefon">{fieldLabel('teilnehmer', 'notfall_telefon')}</Label>
        <Input
          id="notfall_telefon"
    ...
    'bemerkungen_tn': (
      <div key="bemerkungen_tn" className="space-y-1.5">
        <Label htmlFor="bemerkungen_tn">{fieldLabel('teilnehmer', 'bemerkungen_tn')}</Label>
        <Textarea
          id="bemerkungen_tn"
          placeholder=""
          value={fields.bemerkungen_tn ?? ''}
          onChange={e => setFields(f => ({ ...f, bemerkungen_tn: e.target.value }))}
          rows={3}
        />
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "ort": "Ort", "notfall_name": "Name Notfallkontakt", "notfall_telefon": "Telefon Notfallkontakt", "bemerkungen_tn": "Bemerkungen"};
