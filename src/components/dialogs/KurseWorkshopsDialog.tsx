/**
 * KurseWorkshopsDialog — pre-generated create/edit dialog for KurseWorkshops.
 *
 * Props: open, onClose, onSubmit(fields) => Promise<void>, defaultValues?,
 * recordId? (pass when EDITING — enables the attachments section),
 * raeumeList (full hook array — resolves the Raeume applookup),
 * dozentenList (full hook array — resolves the Dozenten applookup),
 * enablePhotoScan?, enablePhotoLocation?.
 *
 * defaultValues is SHAPE-TOLERANT and its prop type is the EXPORTED
 * KurseWorkshopsDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<KurseWorkshops['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<KurseWorkshopsDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { KurseWorkshops, Raeume, Dozenten, LookupValue } from '@/types/app';
...
/** Widened prefill type for KurseWorkshopsDialog.defaultValues — see file header. */
export type KurseWorkshopsDialogDefaults = Omit<KurseWorkshops['fields'], 'kurstyp' | 'niveau' | 'wochentag' | 'status_kurs'> & {
    kurstyp?: LookupValue | string;
    niveau?: LookupValue | string;
    wochentag?: (LookupValue | string)[];
    status_kurs?: LookupValue | string;
  };

interface KurseWorkshopsDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: KurseWorkshops['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: KurseWorkshopsDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kurstyp: LOOKUP_OPTIONS['kurse_workshops']?.['kurstyp'] ?? [],
  niveau: LOOKUP_OPTIONS['kurse_workshops']?.['niveau'] ?? [],
  wochentag: LOOKUP_OPTIONS['kurse_workshops']?.['wochentag'] ?? [],
  status_kurs: LOOKUP_OPTIONS['kurse_workshops']?.['status_kurs'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  raum: APP_IDS.RAEUME,
  dozent: APP_IDS.DOZENTEN,
};
...
export function KurseWorkshopsDialog({ open, onClose, onSubmit, defaultValues, recordId, raeumeList, dozentenList, enablePhotoScan = true, enablePhotoLocation = true }: KurseWorkshopsDialogProps) {
  const [fields, setFields] = useState<Partial<KurseWorkshops['fields']>>({});
  ...
  // Inline-Create state for "Dozenten" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraDozenten` list, and select it in
  // the originating Combobox via the captured `createDozentenField`.
  const [createDozentenOpen, setCreateDozentenOpen] = useState(false);
  const [createDozentenInitial, setCreateDozentenInitial] = useState('');
  const [createDozentenField, setCreateDozentenField] = useState<string>('');
  const [extraDozenten, setExtraDozenten] = useState< Dozenten[]>([]);
  const dozentenListAll = useMemo(
    () => [...dozentenList, ...extraDozenten],
    [dozentenList, extraDozenten],
  );
  function openCreateDozenten(fieldKey: string, q: string) {
    setCreateDozentenField(fieldKey);
    setCreateDozentenInitial(q);
    setCreateDozentenOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['titel', 'kurstyp', 'niveau', 'startdatum', 'status_kurs'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<KurseWorkshops['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'kurse_workshops');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "titel": string | null, // Titel\n  "kurstyp": LookupValue | null, // Typ (select one key: "kurs" | "workshop" | "einzelunterricht") mapping: kurs=Kurs, workshop=Workshop, einzelunterricht=Einzelunterricht\n  "beschreibung": string | null, // Beschreibung\n  "niveau": LookupValue | null, // Niveau (select one key: "anfaenger" | "fortgeschrittene" | "experten" | "alle_niveaus") mapping: anfaenger=Anfänger, fortgeschrittene=Fortgeschrittene, experten=Experten, alle_niveaus=Alle Niveaus\n  "startdatum": string | null, // YYYY-MM-DDTHH:MM\n  "enddatum": string | null, // YYYY-MM-DDTHH:MM\n  "wochentag": LookupValue[] | null, // Wochentag(e) (select one or more keys: "montag" | "dienstag" | "mittwoch" | "donnerstag" | "freitag" | "samstag" | "sonntag") mapping: montag=Montag, dienstag=Dienstag, mittwoch=Mittwoch, donnerstag=Donnerstag, freitag=Freitag, samstag=Samstag, sonntag=Sonntag\n  "uhrzeit_beginn": string | null, // Uhrzeit Beginn\n  "uhrzeit_ende": string | null, // Uhrzeit Ende\n  "raum": string | null, // Display name from Räume (see <available-records>)\n  "dozent": string | null, // Display name from Dozenten (see <available-records>)\n  "max_teilnehmer": number | null, // Maximale Teilnehmerzahl\n  "preis": number | null, // Preis (€)\n  "status_kurs": LookupValue | null, // Status (select one key: "geplant" | "aktiv" | "abgeschlossen" | "abgesagt") mapping: geplant=Geplant, aktiv=Aktiv, abgeschlossen=Abgeschlossen, abgesagt=Abgesagt\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["raum", "dozent"]);
        ...
        return merged as Partial<KurseWorkshops['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
      ...
      <div key="kurstyp" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kurstyp) === 'kurs'}
            onClick={() => setFields(f => ({ ...f, kurstyp: (lookupKey(f.kurstyp) === 'kurs' ? undefined : 'kurs') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kurstyp) === 'kurs'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kurstyp) === 'workshop'}
            onClick={() => setFields(f => ({ ...f, kurstyp: (lookupKey(f.kurstyp) === 'workshop' ? undefined : 'workshop') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kurstyp) === 'workshop'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kurstyp) === 'einzelunterricht'}
            onClick={() => setFields(f => ({ ...f, kurstyp: (lookupKey(f.kurstyp) === 'einzelunterricht' ? undefined : 'einzelunterricht') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kurstyp) === 'einzelunterricht'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
      ...
      <div key="niveau" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.niveau) === 'experten'}
            onClick={() => setFields(f => ({ ...f, niveau: (lookupKey(f.niveau) === 'experten' ? undefined : 'experten') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.niveau) === 'experten'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.niveau) === 'alle_niveaus'}
            onClick={() => setFields(f => ({ ...f, niveau: (lookupKey(f.niveau) === 'alle_niveaus' ? undefined : 'alle_niveaus') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.niveau) === 'alle_niveaus'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
      ...
      <div key="status_kurs" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_kurs) === 'geplant'}
            onClick={() => setFields(f => ({ ...f, status_kurs: (lookupKey(f.status_kurs) === 'geplant' ? undefined : 'geplant') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_kurs) === 'geplant'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_kurs) === 'aktiv'}
            onClick={() => setFields(f => ({ ...f, status_kurs: (lookupKey(f.status_kurs) === 'aktiv' ? undefined : 'aktiv') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_kurs) === 'aktiv'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_kurs) === 'abgeschlossen'}
            onClick={() => setFields(f => ({ ...f, status_kurs: (lookupKey(f.status_kurs) === 'abgeschlossen' ? undefined : 'abgeschlossen') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_kurs) === 'abgeschlossen'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_kurs) === 'abgesagt'}
            onClick={() => setFields(f => ({ ...f, status_kurs: (lookupKey(f.status_kurs) === 'abgesagt' ? undefined : 'abgesagt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_kurs) === 'abgesagt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "kurstyp": "Typ", "beschreibung": "Beschreibung", "niveau": "Niveau", "startdatum": "Startdatum und -uhrzeit", "enddatum": "Enddatum und -uhrzeit", "wochentag": "Wochentag(e)", "uhrzeit_beginn": "Uhrzeit Beginn", "uhrzeit_ende": "Uhrzeit Ende", "raum": "Raum", "dozent": "Dozent", "max_teilnehmer": "Maximale Teilnehmerzahl", "preis": "Preis (€)", "status_kurs": "Status"};
  const CURRENCY_KEYS = new Set<string>(["preis"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"raum": {"raumname": "Raumname", "kapazitaet": "Kapazität (Personen)", "etage": "Etage / Standort", "ausstattung": "Ausstattung", "verfuegbarkeit": "Verfügbarkeit", "bemerkungen_raum": "Bemerkungen"}, "dozent": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "instrumente": "Instrumente / Fachbereiche", "qualifikationen": "Qualifikationen", "beschaeftigungsart": "Beschäftigungsart", "foto": "Foto"}};
  ...
  return (
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.KURSE_WORKSHOPS} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createDozentenOpen && (
      <DozentenDialog
        open={createDozentenOpen}
        onClose={() => setCreateDozentenOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createDozentenEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Dozenten;
            setExtraDozenten(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.DOZENTEN, result.id);
            setFields(prev => ({ ...prev, [createDozentenField]: url } as any));
          }
          setCreateDozentenOpen(false);
        }}
        defaultValues={createDozentenInitial
          ? ({ vorname: createDozentenInitial } as any)
          : undefined}
      />
    )}
    </>
