...
export default function ZahlungenDetailPage() {
  ...
  const [record, setRecord] = useState<Zahlungen | null>(null);
  ...
  async function handleUpdate(fields: Zahlungen['fields']) {
    if (!record) return;
    await LivingAppsService.updateZahlungenEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteZahlungenEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/zahlungen');
  }
  ...
  function getAnmeldungenDisplayName(url?: unknown) {
    ...
    return anmeldungenList.find(r => r.record_id === refId)?.fields.bemerkungen_anmeldung ?? '—';
  }
