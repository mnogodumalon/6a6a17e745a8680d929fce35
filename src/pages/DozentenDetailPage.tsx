...
import { formEnhancements } from '@/config/form-enhancements/Dozenten';
import { evalComputed } from '@/config/form-enhancements/types';
import { t, appLabel, fieldLabel, localeTag, CURRENCY } from '@/i18n';

export default function DozentenDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Dozenten | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getDozenten();
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteDozentenEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/dozenten');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        title={t('not_found')}
        action={
          <Button variant="ghost" onClick={() => navigate('/dozenten')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/dozenten')}
      onEdit={() => setEditing(true)}
      backLabel={t('back')}
      editLabel={t('edit_button')}
    >
      <RecordHeader title={record.fields.vorname ?? appLabel('dozenten')} />
