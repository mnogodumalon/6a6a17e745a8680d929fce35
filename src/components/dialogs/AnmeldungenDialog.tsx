 ...
 * teilnehmerList (full hook array — resolves the Teilnehmer applookup),
 * kurseWorkshopsList (full hook array — resolves the KurseWorkshops applookup),
...
import type { Anmeldungen, Teilnehmer, KurseWorkshops, LookupValue } from '@/types/app';
...
export type AnmeldungenDialogDefaults = Omit<Anmeldungen['fields'], 'status_anmeldung'> & {
    status_anmeldung?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status_anmeldung: LOOKUP_OPTIONS['anmeldungen']?.['status_anmeldung'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  teilnehmer: APP_IDS.TEILNEHMER,
  kurs: APP_IDS.KURSE_WORKSHOPS,
};
...
export function AnmeldungenDialog({ open, onClose, onSubmit, defaultValues, recordId, teilnehmerList, kurseWorkshopsList, enablePhotoScan = true, enablePhotoLocation = true }: AnmeldungenDialogProps) {
  ...
  // Inline-Create state for "KurseWorkshops" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraKurseWorkshops` list, and select it in
  // the originating Combobox via the captured `createKurseWorkshopsField`.
  const [createKurseWorkshopsOpen, setCreateKurseWorkshopsOpen] = useState(false);
  const [createKurseWorkshopsInitial, setCreateKurseWorkshopsInitial] = useState('');
  const [createKurseWorkshopsField, setCreateKurseWorkshopsField] = useState<string>('');
  const [extraKurseWorkshops, setExtraKurseWorkshops] = useState< KurseWorkshops[]>([]);
  const kurseWorkshopsListAll = useMemo(
    () => [...kurseWorkshopsList, ...extraKurseWorkshops],
    [kurseWorkshopsList, extraKurseWorkshops],
  );
  function openCreateKurseWorkshops(fieldKey: string, q: string) {
    setCreateKurseWorkshopsField(fieldKey);
    setCreateKurseWorkshopsInitial(q);
    setCreateKurseWorkshopsOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['teilnehmer', 'kurs', 'anmeldedatum', 'status_anmeldung'] as const;
  ...
  }), [teilnehmerList, kurseWorkshopsList, ]);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="kurs" entity="Kurse & Workshops">\n${JSON.stringify(kurseWorkshopsList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "teilnehmer": string | null, // Display name from Teilnehmer (see <available-records>)\n  "kurs": string | null, // Display name from Kurse & Workshops (see <available-records>)\n  "anmeldedatum": string | null, // YYYY-MM-DD\n  "status_anmeldung": LookupValue | null, // Status (select one key: "angemeldet" | "warteliste" | "storniert" | "abgeschlossen") mapping: angemeldet=Angemeldet, warteliste=Warteliste, storniert=Storniert, abgeschlossen=Abgeschlossen\n  "bemerkungen_anmeldung": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        if (kursName) {
          const kursMatch = kurseWorkshopsList.find(r => matchName(kursName!, [String(r.fields.titel ?? '')]));
          if (kursMatch) merged['kurs'] = createRecordUrl(APP_IDS.KURSE_WORKSHOPS, kursMatch.record_id);
        }
    ...
    'teilnehmer': (
      <div key="teilnehmer" className="space-y-1.5">
        ...
        <Combobox
          ...
          createLabel={t('create_in', { entity: appLabel('teilnehmer') })}
        />
    ...
    'kurs': (
      <div key="kurs" className="space-y-1.5">
        ...
        <Combobox
          id="kurs"
          placeholder=""
          items={kurseWorkshopsListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.titel ?? r.record_id),
          }))}
          value={extractRecordId(fields.kurs)}
          onChange={id => setFields(f => ({ ...f, kurs: id ? createRecordUrl(APP_IDS.KURSE_WORKSHOPS, id) : undefined }))}
          onCreateNew={(q) => openCreateKurseWorkshops("kurs", q)}
          createLabel={t('create_in', { entity: appLabel('kurse_workshops') })}
        />
    ...
    'anmeldedatum': (
      <div key="anmeldedatum" className="space-y-1.5">
        ...
        <DatePicker
          ...
          required
        />
        {showErrors && !fields.anmeldedatum && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'status_anmeldung': (
      <div key="status_anmeldung" className="space-y-1.5">
        <Label htmlFor="status_anmeldung">{fieldLabel('anmeldungen', 'status_anmeldung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_anmeldung) === 'angemeldet'}
            onClick={() => setFields(f => ({ ...f, status_anmeldung: (lookupKey(f.status_anmeldung) === 'angemeldet' ? undefined : 'angemeldet') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_anmeldung) === 'angemeldet'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('anmeldungen', 'status_anmeldung', 'angemeldet') ?? 'Angemeldet'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_anmeldung) === 'warteliste'}
            onClick={() => setFields(f => ({ ...f, status_anmeldung: (lookupKey(f.status_anmeldung) === 'warteliste' ? undefined : 'warteliste') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_anmeldung) === 'warteliste'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('anmeldungen', 'status_anmeldung', 'warteliste') ?? 'Warteliste'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_anmeldung) === 'storniert'}
            onClick={() => setFields(f => ({ ...f, status_anmeldung: (lookupKey(f.status_anmeldung) === 'storniert' ? undefined : 'storniert') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_anmeldung) === 'storniert'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('anmeldungen', 'status_anmeldung', 'storniert') ?? 'Storniert'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_anmeldung) === 'abgeschlossen'}
            onClick={() => setFields(f => ({ ...f, status_anmeldung: (lookupKey(f.status_anmeldung) === 'abgeschlossen' ? undefined : 'abgeschlossen') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_anmeldung) === 'abgeschlossen'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('anmeldungen', 'status_anmeldung', 'abgeschlossen') ?? 'Abgeschlossen'}
          </button>
        </div>
        {showErrors && !fields.status_anmeldung && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'bemerkungen_anmeldung': (
      <div key="bemerkungen_anmeldung" className="space-y-1.5">
        <Label htmlFor="bemerkungen_anmeldung">{fieldLabel('anmeldungen', 'bemerkungen_anmeldung')}</Label>
        <Textarea
          id="bemerkungen_anmeldung"
          placeholder=""
          value={fields.bemerkungen_anmeldung ?? ''}
          onChange={e => setFields(f => ({ ...f, bemerkungen_anmeldung: e.target.value }))}
          rows={3}
        />
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"teilnehmer": "Teilnehmer", "kurs": "Kurs", "anmeldedatum": "Anmeldedatum", "status_anmeldung": "Status", "bemerkungen_anmeldung": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"teilnehmer": {"vorname": "Vorname", "nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "ort": "Ort", "notfall_name": "Name Notfallkontakt", "notfall_telefon": "Telefon Notfallkontakt", "bemerkungen_tn": "Bemerkungen"}, "kurs": {"titel": "Titel", "kurstyp": "Typ", "beschreibung": "Beschreibung", "niveau": "Niveau", "startdatum": "Startdatum und -uhrzeit", "enddatum": "Enddatum und -uhrzeit", "wochentag": "Wochentag(e)", "uhrzeit_beginn": "Uhrzeit Beginn", "uhrzeit_ende": "Uhrzeit Ende", "raum": "Raum", "dozent": "Dozent", "max_teilnehmer": "Maximale Teilnehmerzahl", "preis": "Preis (€)", "status_kurs": "Status"}};
  ...
  return (
    ...
    {createKurseWorkshopsOpen && (
      <KurseWorkshopsDialog
        open={createKurseWorkshopsOpen}
        onClose={() => setCreateKurseWorkshopsOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createKurseWorkshop(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as KurseWorkshops;
            setExtraKurseWorkshops(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.KURSE_WORKSHOPS, result.id);
            setFields(prev => ({ ...prev, [createKurseWorkshopsField]: url } as any));
          }
          setCreateKurseWorkshopsOpen(false);
        }}
        defaultValues={createKurseWorkshopsInitial
          ? ({ titel: createKurseWorkshopsInitial } as any)
          : undefined}
        raeumeList={[]}
        dozentenList={[]}
      />
