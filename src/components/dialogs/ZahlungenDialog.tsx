/**
 * ZahlungenDialog — pre-generated create/edit dialog for Zahlungen.
 *
 * Props: open, onClose, onSubmit(fields) => Promise<void>, defaultValues?,
 * recordId? (pass when EDITING — enables the attachments section),
 * anmeldungenList (full hook array — resolves the Anmeldungen applookup),
 * enablePhotoScan?, enablePhotoLocation?.
 *
 * defaultValues is SHAPE-TOLERANT and its prop type is the EXPORTED
 * ZahlungenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Zahlungen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<ZahlungenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Zahlungen, Anmeldungen, LookupValue } from '@/types/app';
...
/** Widened prefill type for ZahlungenDialog.defaultValues — see file header. */
export type ZahlungenDialogDefaults = Omit<Zahlungen['fields'], 'zahlungsart' | 'zahlungsstatus'> & {
    zahlungsart?: LookupValue | string;
    zahlungsstatus?: LookupValue | string;
  };

interface ZahlungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Zahlungen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: ZahlungenDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  zahlungsart: LOOKUP_OPTIONS['zahlungen']?.['zahlungsart'] ?? [],
  zahlungsstatus: LOOKUP_OPTIONS['zahlungen']?.['zahlungsstatus'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  anmeldung: APP_IDS.ANMELDUNGEN,
};
...
export function ZahlungenDialog({ open, onClose, onSubmit, defaultValues, recordId, anmeldungenList, enablePhotoScan = true, enablePhotoLocation = true }: ZahlungenDialogProps) {
  const [fields, setFields] = useState<Partial<Zahlungen['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['anmeldung', 'betrag', 'zahlungsdatum', 'zahlungsart', 'zahlungsstatus'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Zahlungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'zahlungen');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "anmeldung": string | null, // Display name from Anmeldungen (see <available-records>)\n  "betrag": number | null, // Betrag (€)\n  "zahlungsdatum": string | null, // YYYY-MM-DD\n  "zahlungsart": LookupValue | null, // Zahlungsart (select one key: "ueberweisung" | "barzahlung" | "lastschrift" | "paypal" | "kreditkarte") mapping: ueberweisung=Überweisung, barzahlung=Barzahlung, lastschrift=Lastschrift, paypal=PayPal, kreditkarte=Kreditkarte\n  "zahlungsstatus": LookupValue | null, // Zahlungsstatus (select one key: "ausstehend" | "bezahlt" | "teilbezahlt" | "erstattet") mapping: ausstehend=Ausstehend, bezahlt=Bezahlt, teilbezahlt=Teilbezahlt, erstattet=Erstattet\n  "rechnungsnummer": string | null, // Rechnungsnummer\n  "bemerkungen_zahlung": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        if (anmeldungName) {
          const anmeldungMatch = anmeldungenList.find(r => matchName(anmeldungName!, [String(r.fields.bemerkungen_anmeldung ?? '')]));
          if (anmeldungMatch) merged['anmeldung'] = createRecordUrl(APP_IDS.ANMELDUNGEN, anmeldungMatch.record_id);
        }
        return merged as Partial<Zahlungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
      ...
      <div key="zahlungsart" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsart) === 'barzahlung'}
            onClick={() => setFields(f => ({ ...f, zahlungsart: (lookupKey(f.zahlungsart) === 'barzahlung' ? undefined : 'barzahlung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsart) === 'barzahlung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
      ...
      <div key="zahlungsstatus" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsstatus) === 'teilbezahlt'}
            onClick={() => setFields(f => ({ ...f, zahlungsstatus: (lookupKey(f.zahlungsstatus) === 'teilbezahlt' ? undefined : 'teilbezahlt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsstatus) === 'teilbezahlt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsstatus) === 'erstattet'}
            onClick={() => setFields(f => ({ ...f, zahlungsstatus: (lookupKey(f.zahlungsstatus) === 'erstattet' ? undefined : 'erstattet') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsstatus) === 'erstattet'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
  ...
  const FIELD_LABELS: Record<string, string> = {"anmeldung": "Anmeldung", "betrag": "Betrag (€)", "zahlungsdatum": "Zahlungsdatum", "zahlungsart": "Zahlungsart", "zahlungsstatus": "Zahlungsstatus", "rechnungsnummer": "Rechnungsnummer", "bemerkungen_zahlung": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>(["betrag"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"anmeldung": {"teilnehmer": "Teilnehmer", "kurs": "Kurs", "anmeldedatum": "Anmeldedatum", "status_anmeldung": "Status", "bemerkungen_anmeldung": "Bemerkungen"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.ZAHLUNGEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createAnmeldungenOpen && (
      <AnmeldungenDialog
        ...
        defaultValues={createAnmeldungenInitial
          ? ({ bemerkungen_anmeldung: createAnmeldungenInitial } as any)
          : undefined}
        teilnehmerList={[]}
        kurseWorkshopsList={[]}
      />
