...
function renderVariants(variants, keyOf) {
  ...
  const walk = (list, depth) => {
    ...
    const choice = list[0].choices[depth];
    if (!choice) return call(list[0]);
    const t = list.filter((v) => v.choices[depth].branch);
    const f = list.filter((v) => !v.choices[depth].branch);
    if (!t.length || !f.length) return walk(list.filter((_, i) => i === 0), depth + 1);
    return `${choice.src} ? ${walk(t, depth + 1)} : ${walk(f, depth + 1)}`;
  };
  return `(${walk(variants, 0)})`;
}
...
function apply(filePath, translations, source) {
  ...
  if (spans.length === 0) {
    if (out !== readFileSync(filePath, 'utf8')) writeFileSync(filePath, out);
  ...
  const edits = spans.map((s) => ({ start: s.start, end: s.end, text: s.render(keyOf) }));
