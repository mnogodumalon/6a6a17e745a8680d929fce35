...
import type { Raeume, Dozenten, KurseWorkshops, Teilnehmer, Anmeldungen, Zahlungen, CreateRaeume, CreateDozenten, CreateKurseWorkshops, CreateTeilnehmer, CreateAnmeldungen, CreateZahlungen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async deleteKurseWorkshop(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KURSE_WORKSHOPS}/records/${id}`);
  }
  ...
  static async createZahlungenEntry(fields: CreateZahlungen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.ZAHLUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'zahlungen') });
    return { ...data, record_id: data.id };
  }
  static async updateZahlungenEntry(id: string, fields: Partial<CreateZahlungen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.ZAHLUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'zahlungen') });
    return { ...data, record_id: data.id };
  }
  static async deleteZahlungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ZAHLUNGEN}/records/${id}`);
  }
